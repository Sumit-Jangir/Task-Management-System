...
const Header = ({ setIsUrl }) => {
  ...
  const [selectCheck, setSelectCheck] = useState(["Table","Calendar","Dashboard","Map"]);
  ...
  return (
    <>
      <section
        id="header"
        className="bg-[#00000094] absolute w-full shadow-md py-2 px-3 md:px-6 z-20 flex justify-between items-center"
      >
        <div className="flex items-center">
          <div
            onClick={() => setIsSidebarOpen(true)}
            className="text-white inline-block mr-3 hover:bg-white/20 p-1.5 rounded-sm cursor-pointer"
          >
            <FontAwesomeIcon icon={faBars} />
          </div>
          {isSideBarOpen && <Sidebar setIsSidebarOpen={setIsSidebarOpen} />}
          <Link className="text-xl md:text-2xl text-white font-bold truncate max-w-[120px] md:max-w-none">
            {userDetail.name}
          </Link>
          <div className="hidden lg:inline-block ml-4">
            <NavLink
              className={({ isActive }) =>
                `text-white px-3 py-1 rounded-md ${
                  isActive ? "bg-white !text-[#172b4d]" : "hover:bg-white/20"
                }`
              }
              to="/"
            >
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `text-white ml-2 px-3 py-1 rounded-md ${
                    isActive ? "bg-white !text-gray-900" : "hover:bg-white/20"
                  }`
                }
                to={`/${item.toLowerCase()}`}
              >
            ...
            <button
              ...
              className={`text-white text-lg px-2 pb-[3px] mx-1 rounded-md ${
                showDropdown ? "bg-white !text-gray-900" : "hover:bg-white/20"
              }`}
            >
            ...
            <>
              <div className="inline-block">
                <div
                  onClick={(e) => {
                    handleOpenNavList(e);
                    setIsShowTeam((prev) => !prev);
                  }}
                  className="h-8 w-8 md:h-10 md:w-12 bg-white flex justify-center items-center rounded-md cursor-pointer"
                >
                  <span className="pl-1">{referredUsers?.length + 1}</span>{" "}
                  <FontAwesomeIcon className="mx-1" icon={faUser} />
                </div>
              </div>
              <button
                className="inline-flex px-3 py-1.5 md:px-4 md:py-2 bg-white text-gray-900 rounded hover:bg-gray-200 transition duration-300 items-center text-sm"
